...
def tokenize_multipart_input(
    ...
    if prompt:
        ...
        *xx* represent variables:
        ...
        # "*cls**sent_0**aspect_0*_was*mask*.*sep+*"
        # ['', 'cls', '', 'sent_0', '', 'aspect_0', '_was', 'mask', '.', 'sep+', '']
        for part_id, part in enumerate(template_list):
            ...
            #########################################
            # add sentence, [example.text_a] or [(example.text_a, example.text_b)]
            elif part[:5] == 'sent_':
                sent_id = int(part.split('_')[1])
                new_tokens += enc(input_text_list[sent_id][0]) 
            elif part[:6] == '+sent_':
                # Add space
                sent_id = int(part.split('_')[1])
                new_tokens += enc(' ' + input_text_list[sent_id][0])
                ...
                sent_id = int(part.split('_')[1])
                new_tokens += enc(input_text_list[sent_id][0][:-1])
            elif part[:6] == 'sentl_':
                # Lower case the first token
                sent_id = int(part.split('_')[1])
                text = input_text_list[sent_id][0]
            ...
            if part[:4] == 'sent' or part[1:5] == 'sent':
                ...
                if sent_id == 0:
                    if first_sent_limit is not None:
                        ...
                        ############################################
                        # for Agnews dataset, keep [MASK] in the truncated first sentence
                        # keep the [CLS] token 
            ...
            if segment_plus_1_flag:
                segment_id = segment_id + 1 # if segment_id < 1 else segment_id
    else:
    ...
    assert tokenizer.cls_token_id == input_ids[0], "Warning: [CLS] token is removed"
    assert tokenizer.mask_token_id in input_ids, "Warning: [MASK] token is removed"
...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, template_list, tokenizer, cache_dir=None, mode="train", use_demo=False):
        self.args = args
        ...
        # If using demonstrations, use use_demo=True
        ...
        self.label_list = self.processor.get_labels()  # ["0", "1", "2"]
        self.num_labels = len(self.label_list)
        if args.prompt:
            assert args.mapping is not None
            self.label_to_word = eval(args.mapping)  # {'0':'great','1':'terrible', '2':'okay'}
        ...
        if args.prompt and self.template_list is not None:
            logger.info("There are %d templates. Multiply num_sample by %d" % (len(self.template_list), len(self.template_list)))
            self.num_sample *= len(self.template_list)
        ...
        with FileLock(lock_path):
            ...
            if os.path.exists(cached_features_file) and not args.overwrite_cache:
                ...
                # assert len(self.support_examples) == self.num_labels * args.num_sample
                self.support_examples = self.processor.get_support_examples(args.data_dir)
                ##############################################################################

                if mode == "dev":
                    self.query_examples = self.processor.get_dev_examples(args.data_dir)
                    # assert len(self.query_examples) == self.num_labels * args.num_sample
        ...
        if mode != "train":
            ...
            for query_idx, context_indices, bootstrap_idx in self.example_idx:
                ...
                if self.template_list is not None:
                    template = self.template_list[bootstrap_idx % len(self.template_list)] # Use template in order
                else:
                ...
                self.features.append(self.convert_fn(
                    ...
                    label_list=self.label_list, # ["0", "1", "2"]
                    prompt=args.prompt,
                    template=template,
    ...
    def select_context(self, context_examples):
        ...
        counts = {k: 0 for k in self.label_list} # ["0", "1", "2"]
        if len(self.label_list) == 1:
            # Regression
            counts = {'0': 0, '1': 0}
        selection = []
    ...
    def __getitem__(self, i):
        if self.features is None:
            ...
            if self.template_list is not None:
                template = self.template_list[bootstrap_idx % len(self.template_list)]
            else:
    ...
    def convert_fn(
        ...
        if example.label is None:
            ...
            example_label = label_map[example.label] # 0, 1, 2

        # Prepare other features
        if not use_demo:
            # No using demonstrations
            inputs = tokenize_multipart_input(
                input_text_list=input_example_to_tuple(example), # [example.text_a] or [(example.text_a, example.text_b)]
            ...
            inputs = tokenize_multipart_input(
                input_text_list=augmented_example,  # [[example.text_a], [example.text_a]] or [(example.text_a, example.text_b), (example.text_a, example.text_b)]
                max_length=max_length,
                tokenizer=self.tokenizer,
                task_name=self.args.task_name,
                prompt=prompt,
