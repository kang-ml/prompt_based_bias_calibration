...
class ABSAProcessor(DataProcessor):
    ...
    def get_support_examples(self, data_dir):
        """See base class."""
        filename = os.path.join(data_dir, "support.json")
        with open(filename) as infile:
            support_data = json.load(infile)
        return self._create_examples(support_data, "support")
    ##########################################################
...
class Json_TextClassificationProcessor(DataProcessor):
    ...
    def _create_examples(self, total_data, set_type):
        """Creates examples for the training, dev, test and support sets."""
        examples = []
        text_index = 0
        label_index = 1
        for (i, data) in enumerate(total_data):
