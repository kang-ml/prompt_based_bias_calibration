...
def main():
    ...
    """
    train_length = get_max_length(train_dataset)
    test_length = get_max_length(test_dataset)
    eval_length = get_max_length(eval_dataset)
    print(max(train_length, eval_length, test_length))
    """
    ...
    """
    #######
    # random re-initialize model
    for name, param in model.named_parameters():
        # if "embeddings" not in name:  # Skip initializing embeddings
        param.data.normal_(mean=0.0, std=0.02)  # You can customize mean and std
    #######
    """
    ...
    def confusion_matrix_analysis(num_samples, output, write_output_file=False):
        class_names = json.loads(data_args.class_name) # ['positive', 'negative', 'neutral']/['World', 'Sports', 'Business', 'Technology']
    ...
    def write_predict_test_data_results(label_ids, preds, class_names, original_path, output_path):
        ...
        elif data_args.task_name in ["agnews", "trec", "sst2", "subj", "dbpedia", "snli", "sst-5", "yahoo", "mnli", "qqp", "mrpc", "qnli", "rte", "cola"]:
            output_data = []
            data_id = 0
            text_index = 0
            label_index = 1
    ...
    saved_model_path = os.path.join(training_args.output_dir, f'saved_model_{data_args.test_mode}')
    ###################################
    
    # Training
    if training_args.do_train:
        trainer.train(test_mode=data_args.test_mode, model_path=model_args.model_name_or_path if os.path.isdir(model_args.model_name_or_path) else None)
    ...
    if training_args.do_predict:
        ...
        confusion_matrix_analysis(test_dataset.num_sample, output, data_args.write_output_file)
